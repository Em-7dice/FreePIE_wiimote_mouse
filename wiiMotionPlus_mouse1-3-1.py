...
"""WestleyTwain's Mouse Scrpit 					v:1.3.1
...
def motionplus_update():
	...
	VButton = wiimote[0].buttons.button_down(VolumeButton)
	global pastCutoff
	global rollInit
	global rollCurr
	
	if filters.stopWatch(wiimote[0].buttons.button_down(VolumeButton), 50):
		pastCutoff = True
	if not VButton:
		pastCutoff = False
		rollInit = 0
	...
	if pastCutoff:
		rollCurr = roll_Pos
		if   rollCurr >=  VolumeDegrees + rollInit:
			keyboard.setKeyDown(Key.VolumeDown)
			rollInit = rollCurr
		elif rollCurr <= -VolumeDegrees + rollInit:
